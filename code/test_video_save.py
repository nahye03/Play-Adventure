...
import mission_stamp,mission_clock
import save_page
...
class Page7(ui_functions.Topmenu):
    ...
    def capturePressEvent(self):
        ...
        w = qr.width()
        h = qr.height()
        pyautogui.screenshot(f'my_screen{self.cnt}.png', region=(x, y, w, h))
        print('screenshot')
        self.cnt += 1
    ...
    def dropEvent(self, e):
        ...
        if self.clock_num == 1:
            position = e.pos()
            self.mission_clock.clock1.move(int(values.width * 0.65), int(values.height * 0.6))
            self.mission_clock.clock2.move(int(values.width * 0.16), int(values.height * 0.48))
