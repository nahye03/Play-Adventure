...
def stop(file_name):
    print("녹화 중단!!")
    rec.stop()
    rec.save(values.save_video_path+file_name+".mp4")
    print("영상 저장!!!!!!")
...
class Page6(ui_functions.Topmenu):
    def __init__(self):
        ...
        global rec
        self.setWindowTitle('6. 꾸미기 설명')

        self.video_num = 0
        self.first_cloth = 0

        # self.video_play(values.video_path+values.page6_video[0])
        ...
        self.record_thread = Thread(target=record)
    ...
    def mediaStateChange(self):
        if(self.video_num == 0):
            if (self.mediaPlayer_start.state() == QMediaPlayer.StoppedState):
                ...
                self.overlay_button.finish_button.setEnabled(False)
        ...
        if (self.mediaPlayer.state() == QMediaPlayer.StoppedState):
            if self.video_num == 1:
                ...
                self.overlay_button.finish_button.setEnabled(True)
